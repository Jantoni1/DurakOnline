...
public class ChatBox extends VBox {
     ...
     */
    public void setMessages(ObservableList<Chat> pMessages) {
        mList.setItems(pMessages);
        pMessages.addListener((ListChangeListener<Chat>) pChange -> {
            pChange.next();
     ...
     */
    class MessageListElement extends ListCell<Chat> {
        /**
         * invoked when list is redrawn
         *
         * @param pMessage {@link Chat} to be drawn
         * @param pEmpty   is cell empty
         */
        @Override
        public void updateItem(Chat pMessage, boolean pEmpty) {
            super.updateItem(pMessage, pEmpty);
            HBox box = new HBox();
            if (pMessage != null) {
                Label name = new Label(pMessage.getPlayerName() + ": ");
                Label text = new Label(pMessage.getChatMessage());
